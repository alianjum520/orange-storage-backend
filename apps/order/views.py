...
from .models import UserOrder, StorageFacilityOrder
...
class OrderPlaceView(views.APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            order_data = UserOrder.objects.get(id = data['id'])
            storage_unit_order = data['order']
            stripe_line_items = []
            for storage_unit_order_data in storage_unit_order:
                storage_unit_order = StorageFacilityOrder.objects.get(id = storage_unit_order_data['id'])
                stripe_line_items.append(
                    {
                        'price_data': {
                            'currency':'usd',
                             'unit_amount':(int(storage_unit_order.unit_order.per_unit_price) * storage_unit_order.months)* 100,
                             'product_data':{
                                 'name':storage_unit_order.unit_order.name,
                                 'description': f"Storage facility booked is {storage_unit_order.unit_order.storage_facility.name}"
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                metadata={
                    "order_id":order_data.pk,
                    "user_email":order_data.user.email
                },
                mode='payment',
                success_url='https://orange-storage-frontend.vercel.app/'+ '?success=true',
                cancel_url='https://orange-storage-frontend.vercel.app/' + '?canceled=true',
            )
